...
- [ ] LuCI 状态页尚未提供“一键导入候选”按钮，也未展示实时 RSSI、IP 等附加信息。
- [ ] 切换策略仍采用固定冷却时间，尚未实现指数退避与多轮失败回滚策略。
- [ ] 自动化测试与手工测试矩阵尚未补充。
...
3. README 增补了安装、配置与排障说明，方便使用者快速部署。

### 待办补充

- [ ] 在 LuCI 状态页的扫描结果表格中提供“导入为候选”交互，自动填充 SSID/BSSID。
- [ ] 引入指数退避或逐步延长冷却时间的策略，降低长时间故障时的切换频率。
- [ ] 针对 `relaywatchd` 关键函数补充单元/集成测试脚本，覆盖成功切换与黑名单场景。
