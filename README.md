...
- **扫描缓存加速**：遍历候选网络时会缓存最近一次扫描结果（默认 20 秒 TTL），减少对 `ubus/iwinfo` 的重复调用，兼顾性能与实时性。
...
- `switch_cooldown` 指定切换成功后的基础冷却时间，`switch_backoff_max` 则限定指数退避的最大等待秒数。
- `scan_cache_ttl`（可选）控制无线扫描缓存的寿命，默认 20 秒；设为 0 或负数可关闭缓存以换取实时性。
...
## 性能优化与调优建议
- **扫描缓存 TTL**：守护进程默认缓存 20 秒内的无线扫描结果，可通过调整 `scan_cache_ttl`（若新增该选项）或手动触发扫描来平衡实时性与系统负载。若热点部署环境波动频繁，可适当调低 TTL。
- **日志行数可配**：LuCI 状态页调用的 `action_state` 接口支持传入 `log_lines` 参数（默认 40 行），便于在设备资源受限时控制日志读取成本。
- **黑名单与退避组合**：合理设置 `blacklist_time` 与 `switch_backoff_max`，可避免在热点长时间离线时重复关联导致的 CPU 消耗。
...
- 可执行 `sh tests/relaywatchd/test_relaywatchd.sh` 运行内置单元测试，覆盖黑名单刷新与候选切换的核心路径。建议后续补充关于扫描缓存命中率、手动切换回退等场景的断言，以提升长期运行稳定性。
...
- Cached scan results (20-second TTL by default) to reduce `ubus`/`iwinfo` load while iterating candidates.
